...
var FullPage = /*#__PURE__*/function (_React$Component) {
  ...
  function FullPage(props) {
    ...
    _this.isVerticalScrollIntent = function (changedTouches) {
      ...
      return diffY - _this._touchSensitivity > 0 && diffY >= diffX;
    };

    _this.onTouchMove = function (evt) {
      if (_this.props.scrollMode !== scrollMode.FULL_PAGE || !_this.isVerticalScrollIntent(evt.changedTouches[0])) {
        return;
      }
      ...
      if (evt.path && evt.path.length) {
        var _iterator = _createForOfIteratorHelper(evt.path),
            _step;
            ...
            if (element.nodeType == Node.ELEMENT_NODE) {
              if (element == _this.mainContainerRef.current) {
                ...
                if ((overFlowY == 'auto' || overFlowY == 'scroll') && element.scrollHeight > element.clientHeight) {
                  if (_this._touchStart > touchEnd + _this._touchSensitivity && element.scrollHeight > element.scrollTop + element.clientHeight || _this._touchStart < touchEnd - _this._touchSensitivity && element.scrollTop > 0) {
